...
// TODO: Delete, this is deprecated
 function generateVariantBins(variantData){
 ...
 function generateVariantDataBinsAndDataSets(variantData,ratio){
  let variantBins = [];
  variantData.forEach(variant => {
    let consequence = getConsequence(variant);
    let {type, fmax, fmin} = variant;
 ...
 function renderVariantDescriptions(descriptions){
 ...
 function renderVariantDescription(description){
...
/*  if (ref_allele.length > 20) {
 ...
 function mergeConsequenceColors(colors){
  return 'hotpink';
  // return colors.map( d => {
 ...
 function getColorsForConsequences(descriptions){
...
//  returnObject.consequence =  getConsequence(variant);
 ...
 function getVariantSymbolDetail(variant){
  if(variant.variants){
 ...
 function getVariantSymbol(variant){
  if(variant.variants){
    if(variant.variants.length!==1){
      return variant.variants.length;
    }
 ...
 function getVariantTrackPositions(variantData){
  let presentVariants=[];
