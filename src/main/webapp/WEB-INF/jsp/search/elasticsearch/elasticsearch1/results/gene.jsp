...
<table width="100%">
    <tr><td>
        <div class="results" id="tableDiv" style=";word-wrap: break-word; " >
            <h1>${model.searchBean.species}</h1>
            <table  id="resultsTable" style="width:100%;z-index:999;" >
                <thead>
                <tr><td title="Toggle Check All"><input type="checkbox" onclick="toggle(this)"></td>
                    <td>Species</td>
                    <td>Object</td>
                    <td>Symbol</td>
                    <td>Name/Term/Title</td>
                    <td>Chromosome/Citation</td>
                    <td>Start/ Authors</td>
                    <td>Stop</td>
                    <td>Annotations</td>
                    <td >Strains Crossed</td>
                    ...
                    <c:forEach items="${hitArray}" var="hit">
                        ...
                        <tr onmouseover="this.style.cursor='pointer'" onclick="if (link) window.location.href='${url}'">
                                    ...
                                    <c:choose>
                                        <c:when test="${model.searchBean.species!='' ||  fn:length(model.aggregations.species)==1 || model.searchBean.category.equals('Reference')}">
                                            <td  class="${hit.getSourceAsMap().species}" onmouseover="link=false;" onmouseout="link=true;">
                                                        ...
                                                        <input class="checkedObjects" name="checkedObjects" type="checkbox" value="${hit.getSourceAsMap().term_acc}" data-rgdids="${hit.getSourceAsMap().term_acc}" data-count="${hit.getSourceAsMap().experimentRecordCount}" data-symbol="${hit.getSourceAsMap().symbol}" data-sampleExists="${hit.getSourceAsMap().sampleExists}">
                            ...
                            <td class="${hit.getSourceAsMap().species}" >
                                <c:if test="${hit.getSourceAsMap().species!='All' && hit.getSourceAsMap().species!=null}">
                                    <c:if test="${model.searchBean.species=='' && fn:length(model.aggregations.species)!=1}">
                                        <i class="fa fa-star fa-lg" aria-hidden="true"></i>&nbsp;    ${hit.getSourceAsMap().species}
                                    </c:if>
                            ...
                            <td><span class="${hit.getSourceAsMap().category}">${hit.getSourceAsMap().category} ${hit.getSourceAsMap().subcat}</span></td>
                            ...
                            <td>
                                    ...
                                    ${f:format(hit.getSourceAsMap().citation,t )}
