...
<%}
    relatedCellLines = associationDAO.getAssociatedGenomicElementsForDetailRgdId(obj.getRgdId(), "cell_line_to_cell_line", "originate_from_same_individual_as");
    if( relatedCellLines.size() > 0 ) {
...
%>
<tr>
    <td class="label" valign="top">Originated from same individual as:</td>
    <td><%
        for( GenomicElement ge: relatedCellLines ) { %>
        <a href="<%=Link.ge(ge.getRgdId())%>"><%=ge.getSymbol()%> (<%=ge.getName()%>)</a><br><%
        }%></td>
