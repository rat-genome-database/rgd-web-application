<%
    String totalSeqLength = (String) hitMap.get("totalSeqLength");
    int mapKey = (int) hitMap.get("mapKey");
    Integer chromosomes = Integer.parseInt((String) hitMap.get("chromosomes"));
    int totalGenes = (int) hitMap.get("totalGenes");
%>
<div class="card">
    <div class="card-title" style="background-color: #24609c">
        <!--div class="panel-heading" style="background-color: #6FB98F"-->
        <a href="genomeInformation.html?species=<%=species%>&mapKey=<%=mapKey%>&details=true" id="headerLink<%=species%>" title="click to see more info and other assemblies"><strong style="margin-left:40%;color:white"><%=species%></strong></a>
    </div>
    <div class="card-text" >
            ...
            <% }
            if (species.equals("Green Monkey")) {%>
            ...
            <% }
            if (species.equals("Naked Mole-rat")) { %>
        ...
        <div>
            <table class="table table-striped"> <tr><td>
                <label>Assembly:
                    <select class="$<%=species.replace(" ","")%>Assembly">
                        <%
                        for (String key : assemblyListsMap.keySet()) {
                            if (key.equalsIgnoreCase(species)) {
                        List<edu.mcw.rgd.datamodel.Map> entry = assemblyListsMap.get(species);
                        if (entry != null){
                                for (edu.mcw.rgd.datamodel.Map e : entry){
                                    if (e.getKey()==mapKey){%>
                        <option value="<%=e.getKey()%>" selected><%=e.getName()%></option>
                                <%}
                                else {%>
                        <option value="<%=e.getKey()%>"><%=e.getName()%></option>
                        <%          }
                                }
                                }
                            }
        ...
        <div class="<%=species.replace(" ","")%>Class">
            ...
            <table class="table table-striped">
                <tr><td colspan="2" style="text-align: center">
                    ...
                    <a href="genomeInformation.html?species=<%=species%>&mapKey=<%=mapKey%>&details=true" title="click to see more info and other assemblies"><strong>More Details..</strong></a>
                </td></tr>
                <tr><td>Total Seq Length</td><td><%=totalSeqLength%></td></tr>
                <% if (!species.equals("Squirrel") && !species.equals("Chinchilla")) {%>
                <tr>
                    <td>Chromosomes(haploid)</td>
                    <td><%=chromosomes <= 1 ? "-" : chromosomes%></td>
                </tr>
                <%}
                else {%>
                <tr><td>Scaffolds</td><td></td></tr>
                <% } %>
                <tr><td>Genes</td><td>$<%=totalGenes%></td></tr>
