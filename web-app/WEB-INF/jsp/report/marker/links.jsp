...
<%
    ...
    <tr>
        ...
        <td>JBrowse:
            ...
            <a href="/jbrowse/?data=data_rgd5&loc=<%=fu.getJBrowseLoc(mdPrev)%>&tracks=SSLP">rn5</a>
            <% }
                if( fu.mapPosIsValid(md) ){ %>
            <a href="/jbrowse/?data=data_rgd6&loc=<%=fu.getJBrowseLoc(md)%>&tracks=SSLP">rn6</a>
            <% } %>
        </td>
    </tr>
    <%
    ...
    <tr>
        ...
        <td>JBrowse:
            <% if( fu.mapPosIsValid(mdPrev) ){ %>
            <a href="/jbrowse/?data=data_hg19&loc=<%=fu.getJBrowseLoc(mdPrev)%>&tracks=SSLP">hg19</a>
            <% }
                if( fu.mapPosIsValid(md) ){ %>
            <a href="/jbrowse/?data=data_hg38&loc=<%=fu.getJBrowseLoc(md)%>&tracks=SSLP">hg38</a>
            <% } %>
        </td>
    </tr><%
    }
...
if(obj.getSpeciesTypeKey()==SpeciesType.MOUSE ){
    ...
    <tr>
        ...
        <td>JBrowse:
            <% if( fu.mapPosIsValid(mdPrev) ){ %>
            <a href="/jbrowse/?data=data_mm37&loc=<%=fu.getJBrowseLoc(mdPrev)%>&tracks=SSLP">mm9</a>
            <% }
                if( fu.mapPosIsValid(md) ){ %>
            <a href="/jbrowse/?data=data_mm38&loc=<%=fu.getJBrowseLoc(md)%>&tracks=SSLP">mm10</a>
            <% } %>
        </td>
    </tr><%
        }
