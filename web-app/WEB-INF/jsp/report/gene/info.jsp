...
<table width="100%" border="0" style="background-color: rgb(249, 249, 249)">
  ...
  <%-- show model JBrowse mini chart for genes having positions on current reference assembly --%>
    <% if(fu.mapPosIsValid(md)) {
        ...
        String jbUrl = "http://jbrowse.rgd.mcw.edu/?data="+dbJBrowse+"&tracks="+tracks+"&highlight=&tracklist=0&nav=0&overview=0&loc="+FormUtility.getJBrowseLoc(md);
    %>
    <tr>
        <td  class="label">JBrowse:</td>
        <td align="left">
            <a href="http://jbrowse.rgd.mcw.edu/?data=<%=dbJBrowse%>&loc=<%=fu.getJBrowseLoc(md)%>&tracks=<%=tracks%>">View Region in Genome Browser (JBrowse)</a>
        </td>
    </tr>
    <tr>
